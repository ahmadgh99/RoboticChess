...
class Chessboard:
    def __init__(self,frame, image_processor, UI):
        ...
        self.square_circles = {}  # Representation of the circles on the board
    ...
    def capture_empty_board(self):
        ...
        # Initializes square_circles with the circles in each square
        self.square_circles = ip.detect_circles(self.ip, self.squares, self.warped_img)
    ...
    def detect_changes(self):
        ...
        new_circles = ip.detect_circles(self.ip, self.squares, new_img)
        ...
        if self.stability_hist is None:
            self.stability_hist = new_circles
            return new_moves,promote_to,stab_flag

        for key in self.stability_hist.keys():
    
            if self.stability_hist[key] != new_circles[key]:
                self.stability_counter = 0
                self.stability_hist = new_circles
                return new_moves,promote_to,stab_flag
        ...
        empty_squares_diff = [square for square, state in new_circles.items() if not state and self.square_circles[square]]
        occupied_squares_diff = [square for square, state in new_circles.items() if state and not self.square_circles[square]]
        ...
        if len(empty_squares_diff) == 2 and len(occupied_squares_diff) == 1:
            if self.current_pieces[empty_squares_diff[0]][2] == "Pawn" and \
               self.current_pieces[empty_squares_diff[1]][2] == "Pawn":
                ...
                self.square_circles[captured_piece] = False
        ...
        if len(empty_squares_diff) == 1 and not occupied_squares_diff:
            ...
            for change in differences:
                if change[1] > max_diff:
                   max_diff = change[1]
                   max_square = change
            self.UI.add_message(f"{empty_squares_diff[0]} captured {max_square[0]}")
        ...
        if len(empty_squares_diff) == 2 and len(occupied_squares_diff) == 2:
            ...
            self.square_circles = new_circles.copy()
            self.ip.prev_warped = org_img
            return new_moves,promote_to,stab_flag

        for square, curr_state in new_circles.items():
            prev_state = self.square_circles[square]
    ...
    def recognize_square(self, piece):
        # Recognizes the piece on a given square
