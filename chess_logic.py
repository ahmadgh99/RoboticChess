...
class ChessLogic:
    ...
    def is_capture(self, move):
        from_square, to_square = move[0:2]
    ...
    def get_game_status(self,move):
        ...
        # Create a move object
        chess_move = chess.Move(from_square_index, to_square_index)
        fakeBoard.push(chess_move)
        
        if fakeBoard.is_checkmate():
            self.UI.add_message("Checkmate", font_size = '25sp', Flash = True)
            return None
        elif fakeBoard.is_check():
            self.UI.add_message("Check", font_size = '25sp', Flash = True)
            return None
        elif fakeBoard.is_stalemate():
            self.UI.add_message("Stalemate", font_size = '25sp', Flash = True)
            return None
        elif fakeBoard.is_insufficient_material():
            self.UI.add_message("Draw due to insufficient material", font_size = '25sp', Flash = True)
            return None
        elif fakeBoard.is_seventyfive_moves():
            self.UI.add_message("Draw due to seventy-five moves rule", font_size = '25sp', Flash = True)
            return None
        elif fakeBoard.is_fivefold_repetition():
            self.UI.add_message("Draw due to fivefold repetition", font_size = '25sp', Flash = True)
            return None
        else:
            return "Ongoing"
    ...
    def soundHandler(self,move):
        print("Sound should be played")
        if self.get_game_status(move) is None:
            self.UI.playSound(1)
        elif self.is_capture(move) is not None:
            self.UI.playSound(3)
        else:
            self.UI.playSound(2)
    ...
    class IllegalMoveDone(Exception):
        def __init__(self,moved_from,moved_to):
            self.moved_from = moved_from
            self.moved_to = moved_to

        def print_error():
            return "Illegal Move: {self.moved_from.upper()} to {self.moved_to.upper()}"
